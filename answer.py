# -*- coding: utf-8 -*-
"""
Created on Wed May 26 19:29:32 2021
...
conn.execute("CREATE TABLE IF NOT EXISTS Answer_log_table(question TEXT, context TEXT, answer TEXT, model TEXT,timestamp REAL)")
...
def methods_for_answers():
    ...
    if  request.method =='POST':
        ...
        c.execute("CREATE TABLE IF NOT EXISTS Answer_log_table(question TEXT, context TEXT, answer TEXT, model TEXT,timestamp REAL)")
        c.execute("INSERT INTO Answer_log_table VALUES(?,?,?,?,?)", (data['question'], data['context'],answer, model_name,ts))
        conn.commit()
        ...
        if name:
            c.execute('SELECT * FROM Answer_log_table WHERE model=? AND timestamp >=? AND timestamp <=?',(name,start,end))
            model = c.fetchall()
            output=[]
            for row in model:
            ...
            c.execute('SELECT * FROM Answer_log_table WHERE timestamp >=? AND timestamp <=?',(start,end))
            model = c.fetchall()
            output=[]
            for row in model:
              record = {"timestamp": row[4],
